    ...
    public class EntityEditor : EditorWindow
    {
        ...
        private static void SetMinimalEntityConfiguration(Entity entity)
        {
            if (entity != null)
            {
                ...
                if (type == typeof(System) || type == typeof(RootSystem) || type == typeof(SmellSystem))
                    ...
                    if (debugEditor) Debug.LogWarning("Entity Editor: La unidad no sigue la estructura adecuada, no tiene gameObject padre");
                actualEntity = entity;
            }
        }
        ...
        private static void SetGameObjectEntityReference()
        {
            ...
            if (actualEntity != null)
            {
                ...
                {
                    isSystem = true;
                    if (debugEditor && inChild) Debug.LogWarning("Entity Editor: El script de sistema debería estar en el gameObject actual");
        ...
        private static bool SearchRootSystem()
        {
            ...
            {
                if (debugEditor) Debug.LogWarning("Entity Editor: No existe ningún sistema de juego");
                return false;
            }
            else if (roots.GetLength(0) > 1)
            {
                if (debugEditor) Debug.LogWarning("Entity Editor: Hay más de un root system, se usará el primero");
            }
        ...
        private bool CheckRootReference()
        {
            ...
            {
                GUILayout.TextArea("Entity Editor: No hay ningún root system en la escena\nAñade uno desde el systemic editor", errorTextAreaStyle);
                return false;
            }
            return true;
        }
        ...
        private void OnGUI()
        {
            ...
            if (actualEntity != null)           // Si hemos encontrado una entidad asociada al gameObject la muestra sin dejar editarla
                EditorGUILayout.ObjectField("Entidad concreta", actualEntity, typeof(Entity), true);
            else if (gameObjectEntity != null)  // Si no hay script de entidad pero si gameObject asociado
            {
                ...
                GUILayout.Label("El gameObject " + gameObjectEntity.name + " no tiene un script de unidad\n¿Desea añadir uno?", errorStyle);
                ShowCreateUnitMenu();
            }
            ...
            if (actualEntity != null)
            {
                ...
                if (GUILayout.Button("Actualización Manual")) GetSystemicsComponenetsFromUnit();
                if (GUILayout.Button("Guardar en prefab")) SaveAsAPrefab();
                if (GUILayout.Button("Eliminar Entidad")) DeleteEntity(actualEntity, rootSystem);
            }
        ...
        private void ShowColliderEditor()
        {
            ...
            colliderFoldout = EditorGUILayout.Foldout(colliderFoldout, "Editor de colliders", foldoutStyle);
            if (colliderFoldout)
            {
                newColliderName = EditorGUILayout.TextField("Nombre de collider", newColliderName);

                if (newColliderGameObject == null)
                {
                    if (GUILayout.Button("Añadir gameObject de collider"))
                    {
                ...
                else
                {
                    ...
                    if (!colliderAdded)
                    {
                        ...
                        if (GUILayout.Button("Añadir Collider"))
                        {
                            AddCollider(colliderIndex, newColliderGameObject);
                            colliderAdded = true;
                        }
                    }
                    if (!rigidAdded && GUILayout.Button("Añadir Rigidbody"))
                    {
                    ...
                    if (GUILayout.Button("Terminar Edición"))
                    {
                ...
                {
                    GUILayout.Space(5);
                    GUILayout.Label("Eliminar gameObject colliders", EditorStyles.boldLabel);
        ...
        {
            GUILayout.BeginVertical("Box");
            sysCompFoldout = EditorGUILayout.Foldout(sysCompFoldout, "Editor de componentes sistémicos", foldoutStyle);
            if (sysCompFoldout)
            {
                GUILayout.Label("Componente sistémico a añadir", EditorStyles.boldLabel);
                sysCompSelected = EditorGUILayout.Popup(sysCompSelected, systemicComponentOptions);
                GUILayout.Label("GameObject destino", EditorStyles.boldLabel);
                if (actualEntity.transform.childCount != 0)
                {
                    childIndex = EditorGUILayout.Popup(childIndex, GetChildsNames(actualEntity.transform));
                    if (GUILayout.Button("Añadir Input/Output"))
                        AddSystemicComponent(sysCompSelected, entityComponents[childIndex].gameObject);
                    if (notAdded)
                        GUILayout.TextArea("No se ha añadido el componente sistémico.\nYa existe una instancia de dicho y no" +
                            "se admiten duplicados, deberías tratar de editar la instancia ya existente", errorTextAreaStyle);
                }
                else
                {
                    EditorGUILayout.TextArea("Antes de añadir un componente sistémico debería crear " +
                        "un gameObject destino en el editor de colliders", errorTextAreaStyle);
                }
            }
            GUILayout.EndVertical();
            GUILayout.Space(5);
        ...
        {
            autoUpdate = GUILayout.Toggle(autoUpdate, "Actualización automática");
            gameObjectEntity = (GameObject)EditorGUILayout.ObjectField("Entidad a modificar: ", gameObjectEntity, typeof(GameObject), true);
        ...
        {
            GUILayout.Label("Agrupaciones de inputs/outputs:", EditorStyles.boldLabel);
        ...
        {
            UnityEngine.Object prefab = PrefabUtility.GetPrefabObject(gameObjectEntity);
            if (prefab == null)
            {
                if (!AssetDatabase.IsValidFolder("Assets/SystemicPrefabs"))
                    AssetDatabase.CreateFolder("Assets", "SystemicPrefabs");
                PrefabUtility.CreatePrefab("Assets/SystemicPrefabs/" + gameObjectEntity.name + ".prefab", gameObjectEntity);
            }
            else
                PrefabUtility.ReplacePrefab(gameObjectEntity, PrefabUtility.GetCorrespondingObjectFromSource(gameObjectEntity),
        ...
        private void ShowCreateUnitMenu()
        {
            ...
            if (GUILayout.Button("Añadir script unidad"))
            {
        ...
        {
            EditorGUILayout.ObjectField("Ubicación:", input.gameObject, typeof(GameObject), true);
            SerializedObject obj = new SerializedObject(input);
            SerializedProperty debug = obj.FindProperty("debug");
            EditorGUILayout.PropertyField(debug);
            string[] options = { "Activaciones sin límite", "Activaciones limitadas" };
            int selected = 0;
        ...
        private void ShowDetectionColliderGUI(SerializedProperty detectionCollider, Activable input)
        {
            ...
            {
                GUILayout.Label("No hay collider asociado, deberías añadir uno", errorStyle);
                if (GUILayout.Button("Añadir collider"))
                {
            ...
            else
            {
                ...
                if (collider.gameObject != input.gameObject)
                    GUILayout.Label("El collider de detección no se encuentra en el mismo\nGameObject que el input. Esto impedirá su funcionamiento", errorStyle);
                if (!collider.isTrigger)
                {
                    GUILayout.Label("El collider debería ser de tipo trigger.\nDe otro modo no funcionará el input", errorStyle);
                    if (GUILayout.Button("Convertir a trigger")) collider.isTrigger = true;
                }
            }
        }
        ...
        {
            GUILayout.Label("Estimulos que escucha:");
            ...
            {
                EditorGUILayout.TextArea("No existe ningún estímulo, deberías crear alguno en el editor sistémico", errorTextAreaStyle);
            }
            else
            {
                ...
                if (GUILayout.Button("Añadir Estímulo"))
                {
                ...
                GUILayout.Label("Métodos de activación al entrar al collider para cada estímulo:");
                ShowListProperties(activationMethods);
                if (exit)
                {
                    GUILayout.Label("Métodos para la salida de un estímulo en el collider:");
        ...
        private void ShowOutputBroadcast(Component component)
        {
            ...
            if (!isSystem)
            {
                GUILayout.Label("No deberías haber un Output broadcast\nen una entidad que no sea un sistema", errorStyle);
                if (GUILayout.Button("Eliminar"))
        ...
        private void ShowOutputEmit(Component component)
        {
            ...
            string[] stimuliOptions = rootSystem.StimulusManager.Values.ToArray();
            GUILayout.Space(5);
            GUILayout.Label("Olores que emite la entidad junto con el periodo de emisión");
            if (rootSystem.StimulusManager.Values == null || rootSystem.StimulusManager.Values.Count == 0)
            {
                EditorGUILayout.TextArea("No existe ningún estímulo, deberías crear alguno en el editor sistémico", errorTextAreaStyle);
            }
            else
            {
                for (int i = 0; i < stimuli.arraySize; i++)
                {
                    ...
                    selectedIndex = EditorGUILayout.Popup(selectedIndex, stimuliOptions);
                    prop.stringValue = rootSystem.StimulusManager.Values[selectedIndex];
                    GUILayout.Label("Tiempo de emisión:");
        ...
        private void ShowOutputPresenceActivation(OutputPresenceActivation output)
        {
            ...
            else
            {
                ...
                GUILayout.Label("Objetos estimulables");
                ...
                if (GUILayout.Button("Añadir objeto estimulable"))
                {
