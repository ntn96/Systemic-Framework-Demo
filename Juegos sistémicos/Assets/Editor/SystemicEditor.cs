...
namespace SystemicDesign
{
    ...
    public class SystemicEditor : EditorWindow
    {
        ...
        private static void SearchRootSystem()
        {
            ...
            {
                Debug.LogWarning("Hay más de un root system, se usará el primero");
            }
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Comprobar Integridad de la Jerarquía")) CheckIntegrity();
            if (diagnosisDone)
            {
        ...
        {
            GUILayout.Label("Diagnostico", EditorStyles.boldLabel);
            ...
            if (RootSystem.Instance == null)
            {
                lastDiagnosis.Add("No existe sistema raíz, deberías añadir uno o la jerarquía sistémica no funcionará: UNFIXED");
                unfixedErrors++;
            }
            else
            {
                ...
                {
                    lastDiagnosis.Add("El Root System tenía referencias nulas entre sus sistemas: FIXED");
                    fixedErrors++;
                }
                if (DeleteNullEntities(RootSystem.Instance.AllUnits))
                {
                    lastDiagnosis.Add("El Root System tenía referencias nulas entre sus unidades: FIXED");
                    fixedErrors++;
                }
            }
            for (int i = 0; i < entities.GetLength(0); i++)
            {
                if (entities[i].GetType() != typeof(RootSystem))
                {
                    ...
                    {
                        lastDiagnosis.Add("La entidad " + entities[i].gameObject.name + " no tiene sistema padre: IGNORED");
                        warnings++;
                    }
                    else
                    {
                        if (!entities[i].ParentSystem.Entities.Contains(entities[i]))
                        {
                            ...
                            lastDiagnosis.Add("La entidad " + entities[i].gameObject.name + " no estaba presente en su sistema padre: FIXED");
                            fixedErrors++;
                        }
                        if (entities[i].GetType() == typeof(Unit))
                        {
                            ...
                            {
                                lastDiagnosis.Add("La entidad " + entities[i].gameObject.name + " no estaba presente en el root system: FIXED");
                        ...
                        else
                        {
                            ...
                            {
                                lastDiagnosis.Add("El sistema " + entities[i].gameObject.name + " tenía referencias nulas entre sus entidades: FIXED");
                                fixedErrors++;
                            }
                        }
                    }
            ...
            lastDiagnosis.Add("Fin del diagnostico: alertas " + warnings + ", errores areglados " + fixedErrors + ", errores sin areglar " + unfixedErrors + "\n");
            diagnosisDone = true;
        ...
        private void ShowCreateRootMode()
        {
            ...
            if (GUILayout.Button("Crear Sistema Raiz"))
                CreateRootSystem();
        ...
        private void ShowSystemsMode()
        {
            ...
            GUILayout.EndVertical();
            if (GUILayout.Button("Añadir sistema"))
            {
                SystemCreatorEditor.ShowWindow(rootSystem, window);
            }
            GUILayout.EndVertical();
            if (GUILayout.Button("Eliminar Sistema Raiz"))
            {
                PopupEditor.ShowWindow("De verdad desea borrar el sistema raiz. Esto borrará todos los sistemas. CUIDADO", DeleteRootSystem, null);
            }
            ShowSystemStats();
        }
        ...
        {
            foldoutStats = EditorGUILayout.Foldout(foldoutStats, "Estadísticas", foldoutStyle);
            if (foldoutStats)
            {
                GUILayout.Label("Sistemas: " + rootSystem.AllSystems.Count);
                GUILayout.Label("Unidades: " + rootSystem.AllUnits.Count);
            }
        }
        ...
        {
            stimulusFoldout = EditorGUILayout.Foldout(stimulusFoldout, "Estímulos", foldoutStyle);
            if (stimulusFoldout)
            {
                GUILayout.Label("Número de tipos de estímulos: " + rootSystem.StimulusManager.Values.Count);
        ...
        {
            Undo.RecordObject(rootSystem.StimulusManager, "Añadido un estímulo");
        ...
        private void ShowSystemsList()
        {
            ...
            for (int i = 0; i < rootSystem.AllSystems.Count; i++)
            {
                if (GUILayout.Button("Eliminar"))
                {
                    ...
                    PopupEditor.ShowWindow("De verdad desea borrar el sistema " + actualSystem.gameObject.name +
                        "? Se perderán también todas las entidades asociadas.", EraseSystem, null);
                }
            }
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();
            if (GUILayout.Button("Editar") && selectedSystemIndex >= 0)
        ...
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label("Eliminado recursivo en sistemas", EditorStyles.boldLabel);
            GUILayout.BeginVertical();
            GUILayout.Space(7);
